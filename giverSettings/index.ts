...
export class SimpleGiverOld implements Giver {
  public giverContract: Contract<typeof oldGiverAbi>;

  constructor(ever: ProviderRpcClient, address: string) {
    const giverAddr = new Address(address);
    this.giverContract = new ever.Contract(oldGiverAbi, giverAddr);
  }
  ...
  public async sendTo(sendTo: Address, value: string): Promise<{ transaction: Transaction; output?: {} }> {
    return this.giverContract.methods
      .sendGrams({
        amount: value,
        dest: sendTo,
      }).sendExternal({publicKey: "0x0", withoutSignature: true});
  }
}
